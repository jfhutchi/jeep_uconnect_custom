...
**Grade C.** CarPlay is silicon-plausible but requires hardware-backed MFi authentication and, on the documented QNX 6.6 path, a reversible cabin USB route plus a device-side controller stack. None is proved present or compatible on RA4; licensed receiver software is also missing.

## Exact blocker

Before receiver work, identify a compatible Apple authentication IC and its legitimate integration path, then prove that the active media hub and Radio C2 route can reverse roles with supported VBUS behavior. An existing legacy iPod path is not sufficient evidence.
...
## Hard authentication dependency

Apple's published security architecture ties CarPlay video to MFi-SAP and a custom authentication IC. Software cannot substitute for that authorized hardware boundary. The corpus does not identify a compatible RA4 IC or prove that any legacy iPod authenticator is sufficient.

## Physical and software placement

Public evidence does not locate the later FCA authentication IC in the head unit versus media hub, so placement is UNKNOWN. The documented QNX path places role-swap/device-stack logic in the head-unit software; the RA4 hub must at least be electrically transparent in the reversed state. A separate authorized schematic or part identification is needed to locate the IC.

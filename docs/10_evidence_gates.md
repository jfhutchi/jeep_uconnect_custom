...
| Recovered RA4 filesystem trees | source-image and decoded-tree hashes in the current handoff | complete bounded 106-marker census for media/codec, exact `DeviceProjection.swf`/return events, RA4 AMS/AppManager/Xlet lifecycle, QNX CAR reference lifecycle/notification/audio, legacy QNX 6.6 Apple USB role swap/device mode/DCD, and Harman ModuleLink/servicebroker/projection names | whether the physical stock projection screen, backend/service path, authorized-app lifecycle components, audio/Screen runtime or codec path is present and boot-configured |

...
**[CONFIRMED] Live schema update:** AMS `Installer.copyAndCheck` opens the submitted
JAR itself and splits it. Exact root `xlet.properties` is written to both output
archives; exact uppercase `.MF`, `.SF`, `.RSA`, and `.DSA` suffix members go to
fixed `key.jar`; other members go to the executable named with the incoming
basename. All 135 KIM pairs invert consistently. See
...
**[INFERRED]** `magic.txt` is a generic build/copy sentinel. **[DISPROVED for live AMS authentication]** Installer neither creates nor consumes it; the primary JAR signer path performs authentication.
...
| Upgrade | **[CONFIRMED]** | USB Lua chooses AMS `upgrade` when an app ID is already installed; native catalog installation always invokes the AMS adapter named `upgrade`, including its fresh-install task. AMS delegates to `install` when the app directory is absent. |
...
**[CONFIRMED AMS RECOVERY SEMANTICS]** Jamaica class-object metadata assigns `install(String)Application` (selector `0xAD7BE8`, body/marker `0x5BFCC0`), `recoverProgIfNeeded(String)V` (`0xAD7BF0`, `0x5BFDFD`), and `upgrade(String)Application` (`0xAD7CA8`, `0x5BFE8C`) directly to `Installer`. Install promotes fixed `__newxlet__` to `<appId>`. Upgrade reconciles `prog.bak`, stages a new Xlet, renames current `<appId>/prog` to `prog.bak`, promotes staged `prog`, and removes backup/staging on success; `<appId>/data` persists. Recovery restores `prog.bak` only when `prog` is absent. Filesystem durability and AppManager/QDB/DRM cross-layer reconciliation after power loss remain unknown. The complete evidence and interruption matrix are in `reports/appmanager_registry_atomicity.md` and `reports/resident_incoming_jar_schema.md`.
...
- **[CONFIRMED] Catalog cleanup plus AMS program rollback:** KIM3 `InstallHelper` has staging cleanup and optional uninstall-first behavior. AMS directly owns `prog.bak` promotion/recovery for the program directory. Crash durability and cross-layer native/DRM/QDB reconciliation remain unproved.
...
- **[CONFIRMED AMS program recovery / UNKNOWN cross-layer durability]:** AppManager's AMS callback, resource moves, native-map mutation, and QDB whole-list write are separate phases and can diverge after interruption. AMS has a proved `prog.bak` normal restore path, but no end-to-end A/B slot or cross-layer transaction is proved. Factory KIM file-copy non-atomicity is independently confirmed.

## Open gaps

1. **[UNKNOWN]** A factory external-install manifest that selects `us-app-install.sh`, a sample `<ISO_PATH>/usr/share/APPS` tree, and the authorized issuer/tool interface. The detector, authentication, environment handoff, external-script dispatcher, and live application member schema are confirmed.
...
3. **[UNKNOWN]** Hidden AMS package-registry files/schema, boot reconciliation with the recovered AppManager JSON/QDB catalog, filesystem durability, and interruption behavior across payload, descriptor, native map, resources, DRM, and persistent list. The `prog.bak` parent/use and normal rename recovery are now proved.
